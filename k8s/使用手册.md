...
- ClusterIP: 
  ...
  - 示例：内部数据库服务、内部API服务等。
- NodePort:
  - 用途：如果你需要从集群外部访问服务，可以使用NodePort。它会公开一个随机端口，并将流量转发到Service内部的Pod。
  - 示例：Web应用程序、REST API等。
- LoadBalancer:
  - 用途：如果你需要外部负载均衡器，并希望从集群外部访问服务，可以使用LoadBalancer。云提供商将分配一个外部IP地址和负载均衡器。
  - 示例：Web应用程序、公开的API服务等。
- ExternalName:
  - 用途：如果你需要将服务重定向到集群外部的服务，可以使用ExternalName。它不会创建ClusterIP，只会提供外部服务的DNS别名。
...
查看 Service 提供的在集群内访问pod的ClusterIP（重要！！！）：
```shell
$ kk get service     
NAME                         TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)    AGE
kubernetes                   ClusterIP   20.1.0.1       <none>        443/TCP    7h18m
...
下面启动一个Nginx Pod来访问这个ClusterIP （`20.1.160.186`）。
...
$ kk get service                   
NAME                        TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)          AGE
kubernetes                  ClusterIP   20.1.0.1       <none>        443/TCP          7h52m
...
![k8s-loadbalancer.png](..%2Fimg%2Fk8s-loadbalancer.png)

<img src="../img/k8s-loadbalancer.png" width = "500" height = "800" align=center  alt=""/>
