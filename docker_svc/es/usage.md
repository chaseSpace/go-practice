...
Dev Tools 是 Kibana 的一个工具，可以API操作es数据，比如对索引的增删查改等，很常用。
...
### POST 批量删除数据
POST /t2/_delete_by_query?pretty
{
  "query": {
    "match_all": {}
  }
...
###### 多字段匹配
{
  "query": {
    "multi_match": {
      "query": "Virgin",
      "fields": ["name", "description"],  # 在多个字段中模糊匹配
